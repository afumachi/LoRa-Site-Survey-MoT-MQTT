...
def le_novas_linhas_n4(acumulador):
    ...
    nesse caso. Retorna uma tupla (novas_linhas, houve_reset) - o chamador
    deve truncar/reinicializar os arquivos de SAÍDA do Nível 5 sempre que
    houve_reset for True, ou o gráfico do Nível 6 ficará com dados do teste
    anterior concatenados com os do teste novo.
    """
    novas_linhas = []
    houve_reset = False

    if not os.path.exists(ARQUIVO_N4_APLICACAO):
        return novas_linhas, houve_reset

    tamanho_atual = os.path.getsize(ARQUIVO_N4_APLICACAO)

    # Arquivo foi truncado/recriado (novo teste) -> reseta estatísticas.
    # Cobre tanto "arquivo ficou menor" quanto "arquivo foi recriado vazio
    # e ainda não recebeu nenhuma linha nova" (tamanho_atual == 0 com
    # offset antigo > 0), caso que antes não era detectado aqui.
    if tamanho_atual < acumulador.offset_arquivo_n4 or (
            tamanho_atual == 0 and acumulador.offset_arquivo_n4 > 0):
        acumulador.reset()
        houve_reset = True
...
def main():
    ...
    while True:
        try:
            novas_linhas, houve_reset = le_novas_linhas_n4(acumulador)

            if houve_reset:
                print("[N5-App] Novo teste detectado (arquivo de Nível 4 truncado/recriado). "
                      "Resetando estatísticas e arquivo de saída.")
                caminho_log = inicializa_arquivos_saida()

            # Detecta início de novo teste mesmo sem truncamento explícito
            # (ex.: medida_atual reinicia em 1 após ter avançado bastante,
            # caso em que o tamanho do arquivo pode não ter diminuído a
            # tempo de ser pego pela checagem de truncamento acima)
            if novas_linhas and not houve_reset:
                ...
                if (primeira is not None and acumulador.ultima_medida_atual is not None
                    ...
                    print("[N5-App] Novo teste detectado (medida_atual reiniciou). "
                          "Resetando estatísticas e arquivo de saída.")
                    acumulador.reset()
                    caminho_log = inicializa_arquivos_saida()
