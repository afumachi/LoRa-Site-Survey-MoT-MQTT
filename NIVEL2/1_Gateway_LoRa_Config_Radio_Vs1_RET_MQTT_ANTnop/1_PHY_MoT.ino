...
/*
    // --- Inicia contagem millis se MAC4_COMANDO == 4 ---
    if (recebe_comando_nova_radio != 0) {

      millis_inicio_aguarda_UL  = millis();
      aguardando_confirmacao_UL = true;

//      Serial.print("[GATEWAY COMANDO 4] Aguardando confirmação UL. Tempo limite: ");
//      Serial.print((unsigned long)tempo_radio * 10UL * 1000UL);
//      Serial.println(" ms");

    } else {
      // Comando diferente de 4: garante que não há contagem pendente
      aguardando_confirmacao_UL = false;
      millis_inicio_aguarda_UL  = 0;
    }
*/
...
void Phy_mqtt_send_UL() {
  ...
  PacoteUL[DL_COUNTER_MSB] = (contadorDL >> 8) & 0xFF; // contadorDL/256;
  PacoteUL[DL_COUNTER_LSB] = contadorDL & 0xFF; // contadorDL%256;
