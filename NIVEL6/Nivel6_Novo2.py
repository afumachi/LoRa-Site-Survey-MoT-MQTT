...
# atualizar_grafico_c231 será chamado após todos os widgets serem criados
...
# ld_atualizar_grafico será chamado após todos os widgets serem criados
...
# ds_atualizar_grafico será chamado após todos os widgets serem criados


# =============================================================================
# ABA 6: COMPARAÇÃO – PATH LOSS (todas as curvas)
# =============================================================================
aba_comp_pl = Frame(notebook, bg="#F0F0F0")
notebook.add(aba_comp_pl, text="  📈 Comparação PL  ")

frm_comp_pl_graf = Frame(aba_comp_pl, borderwidth=1, relief='sunken')
frm_comp_pl_graf.place(x=8, y=8, width=1330, height=720)
fig_comp_pl    = Figure(facecolor='white')
canvas_comp_pl = FigureCanvasTkAgg(fig_comp_pl, master=frm_comp_pl_graf)
canvas_comp_pl.get_tk_widget().pack(side=TOP, fill=BOTH, expand=True)

btn_save_comp_pl = Button(aba_comp_pl, text="💾 Salvar Comparação PL",
                          font=("Arial", 10, "bold"), bg="#2dc653", fg="white",
                          activebackground="#1fa83e", cursor="hand2", relief="flat",
                          command=lambda: salvar_grafico(fig_comp_pl, "grafico_comparacao_pl"),
                          padx=8, pady=4)
btn_save_comp_pl.place(x=8, y=735)

def atualizar_grafico_comp_pl(fig, canvas):
    """Aba de comparação: curvas de Path Loss de todos os modelos em um único gráfico."""
    p = coletar_params_todos()
    sens = -174 + 10*math.log10(p['bw']*1e3) + 6 + {7:-7.5,8:-10.0,9:-12.5,10:-15.0,11:-17.5,12:-20.0}.get(p['sf'],-20.0)
    ...
    d_range = [0.3 + i*0.5 for i in range(250)]  # 0.3 m … 125 m

    # Curvas dos três modelos
    pl_mw  = [mw_pl_curva(d, p)                                          for d in d_range]
    pl_ld  = [ld_pl_curva(d, p)                                          for d in d_range]
    pl_ds  = [ds_pl_curva(d, p)                                          for d in d_range]
    pl_fs  = [pl_espaco_livre(d)                                          for d in d_range]

    ax.plot(d_range, pl_fs,  color='#888888', lw=1.5, linestyle=':',  label='Espaço Livre (Friis)')
    ax.plot(d_range, pl_mw,  color='#2E7D32', lw=2.2, linestyle='-',  label='COST 231 Multi-Wall')
    ax.plot(d_range, pl_ld,  color='#6A0DAD', lw=2.2, linestyle='--', label=f'Log-Distance n={p["ld_n"]:.1f}')
    ax.plot(d_range, pl_ds,  color='#E65100', lw=2.2, linestyle='-.',
            label=f'Dual-Slope n₁={p["ds_n1"]:.1f}/n₂={p["ds_n2"]:.1f}/n₃={p["ds_n3"]:.1f}')

    # Faixas de shadowing quando definidas
    if p['ld_sigma'] > 0:
        ax.fill_between(d_range, [v-p['ld_sigma'] for v in pl_ld], [v+p['ld_sigma'] for v in pl_ld],
                        color='#6A0DAD', alpha=0.10, label=f'LD ±σ={p["ld_sigma"]:.1f} dB')
    if p['ds_sigma'] > 0:
        ax.fill_between(d_range, [v-p['ds_sigma'] for v in pl_ds], [v+p['ds_sigma'] for v in pl_ds],
                        color='#E65100', alpha=0.10, label=f'DS ±σ={p["ds_sigma"]:.1f} dB')

    # Linha de link budget
    lb = p['pt'] - sens
    ax.axhline(y=lb, color='#B71C1C', lw=1.4, linestyle='--',
               label=f'Link Budget = {lb:.0f} dB  (Pt={p["pt"]:.0f} dBm, Sens={sens:.0f} dBm)')

    # Linhas de breakpoints do Dual-Slope
    ax.axvline(x=p['ds_d1'], color='#6A0DAD', lw=1.0, linestyle=':', alpha=0.7,
               label=f'BP d₁={p["ds_d1"]:.0f} m')
    ax.axvline(x=p['ds_d2'], color='#1565C0', lw=1.0, linestyle=':', alpha=0.7,
               label=f'BP d₂={p["ds_d2"]:.0f} m')

    # Ponto de medição atual
    ax.scatter([p['d_m']], [mw_pl_curva(p['d_m'],p)],  color='#2E7D32', s=55, zorder=6, marker='o')
    ax.scatter([p['d_m']], [ld_pl_curva(p['d_m'],p)],  color='#6A0DAD', s=55, zorder=6, marker='o')
    ax.scatter([p['d_m']], [ds_pl_curva(p['d_m'],p)],  color='#E65100', s=55, zorder=6, marker='o',
               label=f'd medição = {p["d_m"]:.1f} m')

    ax.set_xlabel('Distância [m]', fontsize=11)
    ax.set_ylabel('Path Loss [dB]', fontsize=11)
    ax.set_title('Comparação de Modelos de Path Loss @ 915 MHz\n'
                 'COST 231 Multi-Wall  |  Log-Distance  |  Dual-Slope',
    ...
    canvas.draw()
    janela_principal.after(REFRESH_MS, atualizar_grafico_comp_pl, fig, canvas)


# =============================================================================
# ABA 7: COMPARAÇÃO – MARGEM DE ENLACE + PL × DISTÂNCIA
# =============================================================================
aba_comp_mg = Frame(notebook, bg="#F0F0F0")
notebook.add(aba_comp_mg, text="  🔗 Margem & PL × d  ")

frm_comp_mg_graf = Frame(aba_comp_mg, borderwidth=1, relief='sunken')
frm_comp_mg_graf.place(x=8, y=8, width=1330, height=720)
fig_comp_mg    = Figure(facecolor='white')
canvas_comp_mg = FigureCanvasTkAgg(fig_comp_mg, master=frm_comp_mg_graf)
canvas_comp_mg.get_tk_widget().pack(side=TOP, fill=BOTH, expand=True)

btn_save_comp_mg = Button(aba_comp_mg, text="💾 Salvar Comparação Margem",
                          font=("Arial", 10, "bold"), bg="#2dc653", fg="white",
                          activebackground="#1fa83e", cursor="hand2", relief="flat",
                          command=lambda: salvar_grafico(fig_comp_mg, "grafico_comparacao_margem"),
                          padx=8, pady=4)
btn_save_comp_mg.place(x=8, y=735)

def atualizar_grafico_comp_mg(fig, canvas):
    """Aba de comparação: margens de enlace DL/UL dos 3 modelos + curva PL×d."""
    p = coletar_params_todos()
    sens = -174 + 10*math.log10(p['bw']*1e3) + 6 + {7:-7.5,8:-10.0,9:-12.5,10:-15.0,11:-17.5,12:-20.0}.get(p['sf'],-20.0)
    lb = p['pt'] - sens

    # Lê medições dos três arquivos de resultado do N5
    lin_mw = ler_resultado_txt(OUT_MULTIWALL)
    lin_ld = ler_resultado_txt(OUT_LOGDISTANCE)
    lin_ds = ler_resultado_txt(OUT_DUALSLOPE)

    fig.clear(); fig.patch.set_facecolor('white')

    # --- Sub-gráfico superior: Margem de enlace por medida (todos os modelos) ---
    ax1 = fig.add_subplot(211); ax1.set_facecolor('#F8F8F8')

    def extrair_margens(linhas, idx_dl=8, idx_ul=9, idx_med=0):
        xs, mg_dl, mg_ul = [], [], []
        for ln in linhas:
            try:
                xs.append(int(ln[idx_med]))
                mg_dl.append(float(ln[idx_dl]))
                mg_ul.append(float(ln[idx_ul]))
            except Exception:
                continue
        return xs, mg_dl, mg_ul

    xs_mw, mg_dl_mw, mg_ul_mw = extrair_margens(lin_mw)
    xs_ld, mg_dl_ld, mg_ul_ld = extrair_margens(lin_ld)
    xs_ds, mg_dl_ds, mg_ul_ds = extrair_margens(lin_ds)

    if xs_mw:
        ax1.plot(xs_mw, mg_dl_mw, color='#2E7D32', lw=1.5, label='MW DL')
        ax1.plot(xs_mw, mg_ul_mw, color='#2E7D32', lw=1.5, linestyle='--', label='MW UL')
    if xs_ld:
        ax1.plot(xs_ld, mg_dl_ld, color='#6A0DAD', lw=1.5, label='LD DL')
        ax1.plot(xs_ld, mg_ul_ld, color='#6A0DAD', lw=1.5, linestyle='--', label='LD UL')
    if xs_ds:
        ax1.plot(xs_ds, mg_dl_ds, color='#E65100', lw=1.5, label='DS DL')
        ax1.plot(xs_ds, mg_ul_ds, color='#E65100', lw=1.5, linestyle='--', label='DS UL')

    for yh, lbl_h, cor_h in [(30,'Excelente','#2E7D32'),(20,'Bom','#F9A825'),
                               (10,'Regular','#E65100'),(0,'Crítico','#B71C1C')]:
        ax1.axhline(y=yh, color=cor_h, lw=0.7, linestyle=':', alpha=0.8, label=f'{lbl_h} ({yh} dB)')

    if not xs_mw and not xs_ld and not xs_ds:
        ax1.text(0.5, 0.5, 'Aguardando dados do Nível 5...', ha='center', va='center',
                 fontsize=12, color='#888888', transform=ax1.transAxes)
        ax1.set_axis_off()
    else:
        ax1.set_ylabel('Margem de Enlace [dB]', fontsize=10)
        ax1.set_xlabel('Medida', fontsize=10)
        ax1.set_title('Margem de Enlace por Medida – Todos os Modelos  (DL sólido / UL tracejado)',
                      fontsize=11, fontweight='bold')
        ax1.legend(loc='upper right', fontsize='x-small', ncol=4)
        ax1.grid(True, linestyle=':', alpha=0.5)

    # --- Sub-gráfico inferior: Curvas PL × distância com margem indicada ---
    ax2 = fig.add_subplot(212); ax2.set_facecolor('#F8F8F8')
    d_range = [0.3 + i*0.5 for i in range(250)]

    pl_mw_cv = [mw_pl_curva(d, p) for d in d_range]
    pl_ld_cv = [ld_pl_curva(d, p) for d in d_range]
    pl_ds_cv = [ds_pl_curva(d, p) for d in d_range]

    ax2.plot(d_range, pl_mw_cv, color='#2E7D32', lw=2.0, linestyle='-',  label='COST 231 Multi-Wall')
    ax2.plot(d_range, pl_ld_cv, color='#6A0DAD', lw=2.0, linestyle='--', label=f'Log-Distance n={p["ld_n"]:.1f}')
    ax2.plot(d_range, pl_ds_cv, color='#E65100', lw=2.0, linestyle='-.', label='Dual-Slope')
    ax2.plot(d_range, [pl_espaco_livre(d) for d in d_range],
             color='#888888', lw=1.2, linestyle=':', label='Espaço Livre')

    ax2.axhline(y=lb, color='#B71C1C', lw=1.4, linestyle='--',
                label=f'Link Budget {lb:.0f} dB')

    # Faixa de margem positiva (área entre link budget e cada curva)
    ax2.fill_between(d_range, pl_mw_cv, lb,
                     where=[v <= lb for v in pl_mw_cv], color='#2E7D32', alpha=0.07)
    ax2.fill_between(d_range, pl_ld_cv, lb,
                     where=[v <= lb for v in pl_ld_cv], color='#6A0DAD', alpha=0.07)
    ax2.fill_between(d_range, pl_ds_cv, lb,
                     where=[v <= lb for v in pl_ds_cv], color='#E65100', alpha=0.07)

    # Marca distância de medição atual
    ax2.axvline(x=p['d_m'], color='#1565C0', lw=1.2, linestyle=':',
                label=f'd={p["d_m"]:.1f} m')

    # Ultima PL medida (DL) de cada modelo como marcador
    if lin_mw:
        try:
            pl_dl_v = float(lin_mw[-1][4])
            ax2.scatter([p['d_m']], [pl_dl_v], color='#2E7D32', marker='D', s=50, zorder=7,
                        label=f'MW PL_DL={pl_dl_v:.1f}')
        except Exception: pass
    if lin_ld:
        try:
            pl_dl_v = float(lin_ld[-1][4])
            ax2.scatter([p['d_m']], [pl_dl_v], color='#6A0DAD', marker='D', s=50, zorder=7,
                        label=f'LD PL_DL={pl_dl_v:.1f}')
        except Exception: pass
    if lin_ds:
        try:
            pl_dl_v = float(lin_ds[-1][4])
            ax2.scatter([p['d_m']], [pl_dl_v], color='#E65100', marker='D', s=50, zorder=7,
                        label=f'DS PL_DL={pl_dl_v:.1f}')
        except Exception: pass

    ax2.set_xlabel('Distância [m]', fontsize=10)
    ax2.set_ylabel('Path Loss [dB]', fontsize=10)
    ax2.set_title('Path Loss × Distância – Comparação de Modelos  (▲ = PL medido DL)',
                  fontsize=11, fontweight='bold')
    ax2.legend(loc='lower right', fontsize='x-small', ncol=3)
    ax2.grid(True, linestyle=':', alpha=0.5)
    ax2.set_xlim(0, max(d_range))

    fig.subplots_adjust(left=0.08, right=0.97, top=0.95, bottom=0.08, hspace=0.45)
    canvas.draw()
    janela_principal.after(REFRESH_MS, atualizar_grafico_comp_mg, fig, canvas)


# =============================================================================
# ABA 8: CONEXÃO SERIAL
# =============================================================================
aba_serial = Frame(notebook, bg="#F0F0F0")
notebook.add(aba_serial, text="  🔌 Conexão Serial  ")  # Aba 8
...
# =============================================================================
# INICIALIZAÇÃO DOS GRÁFICOS – após todos os widgets estarem criados
# (evita NameError por widgets definidos depois das funções de gráfico)
# =============================================================================
...
atualizar_grafico_comp_pl(fig_comp_pl, canvas_comp_pl)
atualizar_grafico_comp_mg(fig_comp_mg, canvas_comp_mg)
