        ...
        try:
            ...
            for line in linhas[1:]:  # ignora o cabeçalho
                ...
                if len(c) >= 7 and c[0] != '':
                    ...
                    resultado['distancia'].append(float(c[4]))
...
criar_botao_salvar_png(aba_mapa_calor, fig_mapa, "mapa_calor_lora_shadowing", x=870, y=10)
...
def _cor_por_rssi_dl(rssi):
    ...
    """
    lim_ruim = _ler_float_seguro(entry_rssi_dl_ruim, -100)
    lim_boa = _ler_float_seguro(entry_rssi_dl_boa, -80)
    lim_exc = _ler_float_seguro(entry_rssi_dl_exc, -60)
    if rssi >= lim_exc:
...
def gerar_mapa_calor(f, c):
    """
    Chamada pelo botão "🔥 Gerar Mapa de Calor". Lê o resultado consolidado
    do Nível5_cobertura.py e desenha: (1) o mapa de cobertura (posição do Nó
    Sensor em cada medida, em metros relativos ao Gateway, colorido pelo
    RSSI Downlink) e (2) o gráfico do modelo Shadowing (RSSI medido x
    distância + curva teórica prevista).
    """
    ...
    # --- Converte lat/lon em coordenadas locais X/Y (metros), relativas
    # ao Gateway, por aproximação equiretangular (adequada para a escala
    # de um Site Survey, tipicamente até poucos km). ---
    R_TERRA = 6371000.0
    xs, ys = [], []
    ...
    # --- Gráfico 1 (topo): Mapa de Calor / Cobertura ---
    ax_mapa = f.add_subplot(211)
    ...
    # --- Gráfico 2 (embaixo): Modelo Shadowing (RSSI x Distância) ---
    ax_shadow = f.add_subplot(212)
    ...
    f.subplots_adjust(left=0.12, bottom=0.08, right=0.95, top=0.94, hspace=0.35)
    c.draw()
...
btn_gerar_mapa = Button(reg_status_mapa, text="🔥 Gerar Mapa de Calor",
                        ...
                        bd=3, state=DISABLED,
                        command=lambda: gerar_mapa_calor(fig_mapa, canvas_mapa))
...
def atualizar_status_mapa_calor():
    """
    Verifica periodicamente (mesmo ciclo de REFRESH_MS das demais abas) se
    o teste está em andamento (PARAMETROS.txt) e se há dados prontos do
    Nível5_cobertura.py, habilitando o botão "Gerar Mapa de Calor" apenas quando
    o teste estiver finalizado e houver ao menos uma medida disponível.
    """
    status_lido = '0'
    path_param = os.path.join(dir_nivel4, 'PARAMETROS.txt')
    if os.path.exists(path_param):
    ...
    n_medidas = len(le_dados_n5_cobertura()['medida'])
    ...
    janela_principal.after(REFRESH_MS, atualizar_status_mapa_calor)
